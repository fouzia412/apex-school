      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {achievements.map((item, index) => (
            <motion.div
              key={item.id}
              className="relative bg-card rounded-2xl p-6 border border-border overflow-hidden shadow-sm"
            ...
            >
              {/* Hover glow */}
              <div
                className={`absolute inset-0 ${item.color} opacity-0 hover:opacity-5 transition-opacity duration-500`}
              />
